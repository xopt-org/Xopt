...
class ScipyOptimizeGenerator(Generator):
    ...
    def x0(self):
        ...
        return  np.array([self.options.initial_point[k] for k in self.vocs.variable_names])
    ...
    def initial_point(self, value):
        if value is None:
            value = self.vocs.random_inputs(
                ...
                include_linked_variables=False)
        self.options.initial_point = value

    def add_data(self, new_data: pd.DataFrame):
        assert len(new_data) == 1, f'length of new_data must be 1, found: {len(new_data)}'
    ...
    def generate(self, n_candidates) -> List[Dict]:
        ...
        if self._lock:
            raise ValueError("Generation is locked via ._lock. "
            "Please call `add_data` before any further generate(1)")

        if n_candidates != 1:
            raise NotImplementedError("simplex can only produce one candidate at a time")

        try:   
            self.x, self.state = next(self._algorithm) # raw x point and any state to be passed back
            self._lock = True
...
class NelderMeadOptions(GeneratorOptions):
    ...
    initial_simplex: Dict[str, List[float]] = None # This overrides the use of initial_point
...
class NelderMeadGenerator(ScipyOptimizeGenerator):
    ...
    def _init_algorithm(self):
        ...
        if options.initial_simplex:
            sim = np.array([self.options.initial_simplex[k] for k in self.vocs.variable_names]).T
        else: 
            sim = None
...
def _neldermead_generator(func, x0, 
                         #args=(), callback=None,
