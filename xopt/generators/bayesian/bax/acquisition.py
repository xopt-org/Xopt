...
class ExpectedInformationGain(AnalyticAcquisitionFunction):
    ...
    def __init__(self, model: Model, algo: type[Algorithm]) -> None:
        ...
        self.algo = algo
        self.xs_exe, self.ys_exe = self.algo.get_exe_paths(
            self.model
        )  # get sampled execution paths and set self.xs_exe, self.ys_exe
...
class PosteriorUncertainty(AnalyticAcquisitionFunction):
    r"""Single outcome expected information gain`
        Currently only works model = SingleTaskGP (not ModelListGP)
    Example:
        >>> model = SingleTaskGP(train_X, train_Y)
        >>> EIG = ExpectedInformationGain(model, algo, algo_params)
        >>> eig = EIG(test_X)
