...
class TestBayesianGenerator(TestCase):
    ...
    def test_get_training_data(self):
        ...
        onames = list(TEST_VOCS_BASE.objectives.keys()) + \
                 list(TEST_VOCS_BASE.constraints.keys())

        true_inputs = torch.from_numpy(
            TEST_VOCS_DATA[inames].to_numpy()
        )
        true_outputs = torch.from_numpy(
            TEST_VOCS_DATA[onames].to_numpy()
        )
